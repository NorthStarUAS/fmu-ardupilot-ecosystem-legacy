...
bool affine_from_points(Eigen::MatrixXf src, Eigen::MatrixXf dst, bool shear, bool scale, Eigen::MatrixXf &M ) {
    console->printf("affine_from_points()\n");
    int ndims = src.cols();
    if ( src.rows() != dst.rows() or src.cols() != dst.cols() ) {
        console->printf("src and dst are different dimensions\n");
        return false;
    }
    if ( ndims < 2 or src.cols() < ndims ) {
        console->printf("need at least 2 dimensions.\n");
        return false;
    }
    ...
    if ( shear ) {
        ...
        print_matrix("R", R);
        console->printf("det(R): %.8f\n", R.determinant());
        if ( R.determinant() < 0.0 ) {
            console->printf("Not a right handed system, fixme!\n");
            return false;
        }
    ...
    M = M1.inverse() * M * M0;
    print_matrix("M", M);
    console->printf("M[ndims,ndims]: %.4f\n", M(ndims,ndims));
    M /= M(ndims,ndims);
